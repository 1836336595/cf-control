  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::crazyswarm::TrajectoryPolynomialPiece_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.poly_x.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.poly_x[i]);
    }
    if (v.poly_x.empty() || true)
      s << "]";
    ...
    for (size_t i = 0; i < v.poly_y.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.poly_y[i]);
    }
    if (v.poly_y.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.poly_z.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.poly_z[i]);
    }
    if (v.poly_z.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.poly_yaw.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.poly_yaw[i]);
    }
    if (v.poly_yaw.empty() || true)
      s << "]";
    if (true || !indent.empty())
